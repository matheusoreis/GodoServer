...
export class CreateAccountCore {
  ...
  public async create(): Promise<void> {
    ...
    try {
      ...
      if (account) {
        new AlertCore(this.connection, 'Já existe uma conta com este e-mail.', false).send();
        return;
      }

      const hashedPassword = await this.hash.hash(this.password);
      ...
      new AlertCore(this.connection, 'Sua conta foi criada com sucesso!', false).send();
