...
import { MapCharactersToCore } from '../map-characters-to/map-characters-to.core';
import { NewCharacterToCore } from '../new-character-to/new-character-to.core';

export class WorldOutgoing {
  public sendCharacterSelected(connection: Connection, character: Character): void {
  ...
  public sendExistingCharacters(connection: Connection, memory: Memory): void {
    const characters = memory.worldsCharacters.filter((c) => c !== undefined) as Character[];
    new MapCharactersToCore(connection, characters).send();
  }
}
